...
39. 읽기DB를 분리하기 위해 kafka source connector가 각 테이블을 모니터링하고 있고 여러 테이블을 동시에 listen할 수 없음. 따라서 하나의 테이블이 읽기DB가 필요로 하는 많은 값들을 더 가지고 있어야 하고, 따라서 각 테이블마다 중복되는 컬럼 값이 많아짐. 피할 수 없을 것 같음.
